    ...
    if (robots[i].speed > 0)
    {
      ...
      for (n = 0; n < MAXROBOTS; n++)
      {
        if (robots[n].status == DEAD || i == n)
          continue;
        if (abs(static_cast<int>(robots[i].x - robots[n].x)) < CLICK &&
  ...
  for (r = 0; r < MAXROBOTS; r++)
  {
    ...
    for (i = 0; i < MIS_ROBOT; i++)
    {
      ...
      if (missiles[r][i].stat == FLYING)
      {
        ...
        missiles[r][i].cur_x = x = (int)(missiles[r][i].beg_x + (int)(lcos(missiles[r][i].head) * (long)(missiles[r][i].curr_dist / CLICK) /
                                                                      10000L));
        missiles[r][i].cur_y = y = (int)(missiles[r][i].beg_y + (int)(lsin(missiles[r][i].head) * (long)(missiles[r][i].curr_dist / CLICK) /
                                                                      10000L));
