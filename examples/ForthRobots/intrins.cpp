...
long c_cannon(long num, long degree, long distance)
{
  assert(num >= 0 && num <= 3);
  struct robot *cur_robot = &robots[num];
...
long c_damage(long num)
{
  struct robot *cur_robot = &robots[num];
  return ((long)cur_robot->damage);
}

/* c_speed - report current speed */

long c_speed(long num)
{
  struct robot *cur_robot = &robots[num];
  return ((long)cur_robot->speed);
}

/* c_loc_x - report current x location */

long c_loc_x(long num)
{
  struct robot *cur_robot = &robots[num];
  return ((long)cur_robot->x / CLICK);
}

/* c_loc_y - report current y location */

long c_loc_y(long num)
{
  struct robot *cur_robot = &robots[num];
  return ((long)cur_robot->y / CLICK);
}

/* c_rand - return a random number between 0 and limit */
/*          expect one argument, limit */

long c_rand(long limit)
{
  if (limit <= 0L)
    return (0L);
  else
    return ((long)((long)(rand()) % limit));
}

/* c_sin - return sin(degrees) * SCALE */
/*         expect one agrument, degrees */

long c_sin(long degree)
{
  degree = degree % 360L;
...
long c_atan(long ratio)
{
  long degree = (long)(atan((double)ratio / SCALE) * RAD_DEG);
